...
def generate_dataset_2class(pos_edge_index, neg_edge_index, splits=5, test_prob=0.2, instance_balancing=False, ratio=3, device='cuda:3'):
    datasets = {}
    ...
    for i in range(splits):
        ...
        train_p = train_triple[:, 2] > 0
        train_n = train_triple[:, 2] <= 0
        train_pos_edge_index = train_triple[train_p, :2].astype(int)
        train_neg_edge_index = train_triple[train_n, :2].astype(int)
        test_p = test_triple[:, 2] > 0
        test_n = test_triple[:, 2] <= 0
        test_pos_edge_index = test_triple[test_p, :2].astype(int)
        test_neg_edge_index = test_triple[test_n, :2].astype(int)

        val_pos_index = random.sample(range(len(train_pos_edge_index)), 200)
        val_neg_index = random.sample(range(len(train_neg_edge_index)), 20)

        val_pos_edge_index = train_pos_edge_index[val_pos_index][:]
        val_neg_edge_index = train_neg_edge_index[val_neg_index][:]

        np.delete(train_pos_edge_index, val_pos_index, axis=0)
        np.delete(train_neg_edge_index, val_neg_index, axis=0)

        train_pos = torch.from_numpy(train_pos_edge_index).t().to(device)
        train_neg = torch.from_numpy(train_neg_edge_index).t().to(device)
        val_pos = torch.from_numpy(val_pos_edge_index).t().to(device)
        val_neg = torch.from_numpy(val_neg_edge_index).t().to(device)
        test_pos = torch.from_numpy(test_pos_edge_index).t().to(device)
        test_neg = torch.from_numpy(test_neg_edge_index).t().to(device)
